    ...
    <div className="min-h-screen" style={{ background: "#f8fafc" }}>
      ...
      <article className="max-w-3xl mx-auto px-8 py-12">
        ...
        <Section title="2. What we collect">
          ...
          <p><strong>Attachments (photos, PDFs, files, links).</strong> If you choose to attach a file or photo to a chat message, the content is sent to Anthropic&apos;s Claude API along with your question to generate a response. Photos and file contents are <em>not</em> stored on our servers after the chat session; the analyzed Learning DNA may reference what was discussed but does not retain the original media.</p>
          <p><strong>Camera capture.</strong> If you use the &ldquo;Take a Photo&rdquo; feature, your browser will ask for camera permission. Captured images are processed the same way as any uploaded photo — sent to Claude for that single response, then discarded. We never record video and never access your camera unless you explicitly click &ldquo;Capture.&rdquo;</p>
          <p><strong>Teacher-uploaded assignments.</strong> When a teacher adds an assignment (worksheet PDF or pasted text) to a class, the assignment content is stored in our database so students in that class can be tutored on it. The assignment is only visible to teachers and students of that class. Deleting an assignment removes it from our database.</p>
        </Section>
